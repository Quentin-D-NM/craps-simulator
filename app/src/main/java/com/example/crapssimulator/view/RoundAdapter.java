package com.example.crapssimulator.view;
...
public class RoundAdapter extends ArrayAdapter<Roll> {
  ...
  private State state;
  @ColorInt final int winningRound;
  @ColorInt final int losingRound;
  ...
  public RoundAdapter(Context context) {
    ...
    for (int i = 0; i < faces.length; i++) {
      faces[i] = ContextCompat.getDrawable(context, res.getIdentifier("face_" + (i + 1), "drawable", pkg ));
    }
  ...
  public View getView(int position, View view, ViewGroup parent) {
    ...
    ((TextView) view.findViewById(R.id.value)).setText(getContext().getString(R.string.value_format, value));
